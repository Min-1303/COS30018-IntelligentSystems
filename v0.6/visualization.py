...
def plot_sentiment_vs_stock_price(merged_data):
    plt.figure(figsize=(10, 5))
    plt.plot(merged_data['date'], merged_data['Close'], label='Stock Price')
    plt.plot(merged_data['date'], merged_data['positive'], label='Positive Sentiment', linestyle='--')
    plt.plot(merged_data['date'], merged_data['neutral'], label='Neutral Sentiment', linestyle=':')
    plt.plot(merged_data['date'], merged_data['negative'], label='Negative Sentiment', linestyle='-.')
